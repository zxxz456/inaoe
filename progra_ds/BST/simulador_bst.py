...
Description:
------------
Simulador interactivo de un Arbol Binario de Busqueda (BST). Recibe una
secuencia de valores por linea de comandos, los pasa a una pagina HTML y
genera una animacion interactiva en el navegador que muestra, paso a paso,
como se construye el arbol. Dentro de la misma pagina aparecen casillas para
INSERTAR, BUSCAR y ELIMINAR nuevos valores, cada una con su animacion.

La estructura del arbol esta en `bst.py` (clases TreeNode y BinarySearchTree).
Como las operaciones interactivas se ejecutan en el navegador, la misma logica
esta reflejada en JavaScript dentro de esta pagina (insercion, busqueda y la
eliminacion estandar con sus tres casos: hoja, un hijo y dos hijos/sucesor).


Usage:
------
    python3 simulador_bst.py 50 30 70 20 40 60 80 10 25
    python3 simulador_bst.py 8 3 10 1 6 14 4 7 13 --no-open -o arbol.html
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Simulador interactivo de un Arbol Binario de Busqueda.")
    parser.add_argument("valores", nargs="*",
                        help="valores iniciales a insertar, en orden (ej: 50 30 70)")
    ...
    # Evitar mezclar int y str (no son comparables en Python 3).
    ...
    # Se guarda en el directorio actual (no en /tmp) para que el navegador,
    # sobre todo Firefox tipo snap en Ubuntu, pueda acceder al archivo.
    out = args.output if args.output else "bst_animacion.html"
