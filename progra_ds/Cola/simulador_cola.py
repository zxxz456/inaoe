...
Description:
------------
Simulador interactivo de una Cola (Queue, FIFO). Recibe una secuencia de
...
construye la cola. Dentro de la misma pagina aparecen botones para
ENQUEUE, DEQUEUE, PEEK, BUSCAR y RECORRER, cada una con su animacion.

La estructura de la cola esta en `cola.py` (clase Cola). Como las
...
------
    ...
    python3 simulador_cola.py a b c d --no-open -o cola.html
...
def generate_html(keys):
    ...
    Inputs:
    ------
    keys: Lista de valores iniciales (int o str) a precargar en la cola.
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Simulador interactivo de una Cola (Queue, FIFO).")
    parser.add_argument("valores", nargs="*",
                        help="valores iniciales a encolar, en orden "
                             "(el primero queda en el frente)")
    ...
    out = args.output if args.output else "cola_animacion.html"
