...
\subsection*{Métodos para resolver recurrencias}

Resolver una recurrencia es encontrarle una forma cerrada, o al menos
una cota asintótica. Se vieron cuatro caminos.
...
\begin{enumerate}[itemsep=0.2em]
  ...
  \item \textbf{Probarla por inducción} matemática, y de paso
        encontrar las constantes que la hacen cierta.
\end{enumerate}

El nombre viene del segundo paso: se sustituye la hipótesis de
inducción dentro de la recurrencia. El método no dice cómo adivinar,
y esa es su debilidad, hay que llegar con la respuesta propuesta. Por
eso se usa junto con los árboles de recursión, que sirven para
producir la conjetura.

\begin{ejemplo}
  Resolver $T(n) = 2\,T(n/2) + n$.

  \textbf{Paso 1.} Conjetura: $T(n) = \Ord{n \log n}$. Escrito como
  lo va a usar la inducción, existe $c > 0$ tal que
  $T(n) \leq c\,n \log n$.

  \textbf{Paso 2.} Se supone cierto para $n/2$, o sea
  $T(n/2) \leq c\,(n/2) \log(n/2)$, y se sustituye:
  \begin{align*}
    T(n) &\leq 2 \left( c\,\frac{n}{2} \log \frac{n}{2} \right) + n \\
         ...
         &= c\,n \log n - c\,n + n .
  \end{align*}
  Ese último renglón es $\leq c\,n \log n$ siempre que
  $-c\,n + n \leq 0$, es decir $c \geq 1$. Con eso el paso inductivo
  cierra.
\end{ejemplo}

\begin{ojo}
  El caso base de esta inducción no puede ser $n = 1$: ahí
  $c \cdot 1 \cdot \log 1 = 0$, y $T(1) \leq 0$ es falso porque
  $T(1)$ es una constante positiva. No es que la conjetura esté mal,
  es que la cota arranca más arriba. Se toman $n = 2$ y $n = 3$ como
  base, y basta escoger $c$ grande, porque la notación asintótica
  solo exige que la desigualdad valga de un $n_0$ en adelante.
\end{ojo}

\begin{ojo}
  La inducción tiene que probar \emph{exactamente} la forma que se
  conjeturó, con la misma constante. Un error clásico es probar
  $T(n) \leq c\,n$ dejando un término suelto y decir que ese término
  se absorbe porque es de orden menor. No se absorbe: la hipótesis de
  inducción se usa como está escrita, y si no cierra, hay que
  fortalecerla restando un término de orden menor.
\end{ojo}

\subsubsection*{Método de iteración}

En vez de adivinar, se desenrolla. Se sustituye la recurrencia dentro
de sí misma una y otra vez hasta que el patrón se vuelve visible,
luego se suma la serie que quedó y se aplica el caso base para saber
dónde parar.

\begin{ejemplo}
  Sobre la misma $T(n) = 2\,T(n/2) + n$:
  \begin{align*}
    T(n) &= 2\,T(n/2) + n \\
         &= 2 \left[ 2\,T(n/4) + n/2 \right] + n
          = 4\,T(n/4) + 2n \\
         &= 4 \left[ 2\,T(n/8) + n/4 \right] + 2n
          = 8\,T(n/8) + 3n .
  \end{align*}
  Después de $i$ desdoblamientos queda
  $T(n) = 2^i\,T(n/2^i) + i\,n$. El descenso para cuando el argumento
  llega al caso base, $n/2^i = 1$, o sea $i = \log_2 n$. Sustituyendo,
  \[
    T(n) = n\,T(1) + n \log_2 n = \Ord{n \log n} .
  \]
\end{ejemplo}

No hace falta conjeturar nada, pero el álgebra se pone pesada rápido
y es fácil equivocarse al identificar el patrón general.

\subsubsection*{Árboles de recursión}

Es la iteración, pero dibujada. Cada nodo es el costo que ese nivel
paga por su cuenta, sin contar las llamadas, y los hijos son los
subproblemas. Se suma el costo por nivel, se cuenta cuántos niveles
hay, y se suman los niveles.
...
\begin{tabular}{cccc}
  \toprule
  \textbf{Nivel} & \textbf{Subproblemas} & \textbf{Tamaño}
...
Cada nivel cuesta $n$ porque los subproblemas se duplican al mismo
ritmo que se encogen, y hay $\log_2 n + 1$ niveles. El total es
$n(\log_2 n + 1) = \Ord{n \log n}$.

\begin{idea}
  El árbol no es una prueba, es una herramienta para conjeturar. Se
  dibuja para ver a qué le va a dar la respuesta, y luego esa
  conjetura se prueba formalmente con sustitución. Los dos métodos se
  usan en pareja.
\end{idea}
...
\]
que es el molde de divide y vencerás: $a$ subproblemas de tamaño
$n/b$, más $f(n)$ de costo por partir y juntar.

Todo se decide comparando $f(n)$ contra $n^{\log_b a}$, que es lo que
cuestan las hojas del árbol:

\begin{itemize}[itemsep=0.2em]
  \item Si $f(n)$ crece más lento, mandan las hojas y
        $T(n) = \Tht{n^{\log_b a}}$.
  \item Si crecen igual, todos los niveles cuestan lo mismo y
        $T(n) = \Tht{n^{\log_b a} \log n}$.
  \item Si $f(n)$ crece más rápido, manda la raíz y
        $T(n) = \Tht{f(n)}$.
\end{itemize}

Para quick sort balanceado, $a = 2$ y $b = 2$, así que
$n^{\log_2 2} = n$, que es justo $f(n) = n$. Es el segundo caso, y
devuelve $\Tht{n \log n}$ sin hacer cuentas.

\begin{center}
\begin{tabular}{lll}
  \toprule
  \textbf{Método} & \textbf{Qué pide} & \textbf{Qué da} \\
  \midrule
  ...
  Maestro     & Que encaje en el molde  & La cota, directo \\
  \bottomrule
\end{tabular}
\end{center}

\begin{pendiente}
  Del método maestro solo se dio el enunciado. Faltan las condiciones
  finas de cada caso, el hueco entre los casos donde el teorema no
  aplica, y por qué el tercero necesita además una condición de
  regularidad sobre $f$.
\end{pendiente}
